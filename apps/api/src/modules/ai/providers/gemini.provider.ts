...
export class GeminiProvider implements IAIProvider {
  ...
  async generateResponse(
    ...
    if (!apiKey) {
      throw new Error('Gemini API Key is missing. Please configure it in Workspace Settings.');
    }
    ...
    try {
      ...
      const chat = model.startChat({
        history,
        systemInstruction: systemPrompt ? { role: 'system', parts: [{ text: systemPrompt }] } as any : undefined,
      });
